...
export default defineConfig(({ command }) => {
    // __dirname is /Users/[username]/git/js-api-samples/samples/
    ...
    if (command === 'build') {
        ...
        const workspaceDir = process.cwd(); // e.g., /Users/[username]/git/js-api-samples/samples/test-example
        ...
        if (
            workspaceDir.startsWith(samplesBaseDir) &&
            workspaceDir !== samplesBaseDir
        ) {
            ...
            effectiveBuildInput['index'] = resolve(workspaceDir, 'index.html');
        } else {
            ...
            console.warn(
                `Vite build initiated from unexpected directory: ${workspaceDir}. Defaulting output to root of dist.`
            );
    ...
    return {
        root: process.cwd(), // Always use the sample directory as the root
        plugins: [
            {
                name: 'html-transform',
                enforce: 'pre',
        ...
        build: {
            ...
            rollupOptions:
                command === 'build' &&
                Object.keys(effectiveBuildInput).length > 0
                    ? { input: effectiveBuildInput }
                    : undefined,
        },
        preview: {
            port: 8080,
