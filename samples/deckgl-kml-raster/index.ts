...
async function initMap(): Promise<void> {
   ...
   onDataLoad: (data) => {
     ...
     if (data && data.features) {
       data.features.forEach((feature: any) => {
         if (feature.properties && feature.properties.visibility !== undefined) {
           console.log(`Feature "${feature.properties.name}" visibility: ${feature.properties.visibility}`);
         }
       });
     }
 ...
 const iconLayer = new deck.IconLayer({
   ...
   pickable: true,
   onDataLoad: (data) => {
     console.log('IconLayer data loaded:', JSON.stringify(data, null, 2));
   },
   getPosition: (d: any) => {
 ...
 const textLayer = new deck.TextLayer({
   ...
   data: geojsonLayer.props.data, // Use the same data as the GeoJsonLayer
   onDataLoad: (data) => {
     console.log('TextLayer data loaded:', JSON.stringify(data, null, 2));
   },
   getPosition: (d: any) => {
