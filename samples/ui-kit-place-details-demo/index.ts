...
async function init(): Promise<void> {
  ...
  const [, placesLib] = await Promise.all([
    ...
    google.maps.importLibrary('places') as Promise<typeof google.maps.places>,
    google.maps.importLibrary('marker'),
  ]);

  const placeDetailsWidget = document.querySelector<HTMLElement>('#place-details-widget')!;
  const placeDetailsRequest = document.querySelector<HTMLElement>('#place-details-request')!;
  const addressInput = document.querySelector<HTMLInputElement>('#address')!;
  const autocompleteList = document.querySelector<HTMLElement>('#autocomplete-list')!;
  ...
  function applyContentConfig(mode: string | null) {
    ...
    const currentPlace = placeDetailsRequest.getAttribute('place') || 'places/ChIJ5abCmkWHhYARH3zgiLVc_Ew';
    placeDetailsWidget.innerHTML = '';
    
    const newRequest = document.createElement('gmp-place-details-place-request');
    ...
    if (mode === 'standard') {
      ...
      const customConfig = document.createElement('gmp-place-content-config');
  ...
  themeButtons.forEach((button) => {
    button.addEventListener('click', () => {
      ...
      placeDetailsWidget.classList.remove('theme-google', 'theme-miami', 'theme-redwood');
        ...
        if (response?.suggestions && response.suggestions.length > 0) {
          currentSuggestions = response.suggestions
            ...
            .map((s) => {
              ...
              const fullText =
                typeof pred.text === 'string' ? pred.text : pred.text?.text ?? mainText;

              let distanceText: string | undefined;
              if (pred.distanceMeters && pred.distanceMeters > 0) {
                distanceText = `${(pred.distanceMeters / 1609.34).toFixed(1)} mi`;
              }

              const mainMatches = (pred.mainText?.matches || []).map((m) => ({
  ...
  function renderDropdown(): void {
    ...
    currentSuggestions.forEach((item, index) => {
      ...
      titleEl.innerHTML = buildHighlightedText(item.mainText, item.mainMatches);
    ...
    footer.innerHTML = '<span class="gmp-attribution-text">Google Maps</span>';
  ...
  function selectSuggestion(item: SuggestionItem): void {
    ...
    const activeRequest = document.querySelector<HTMLElement>('#place-details-request');
    if (activeRequest) {
  ...
  addressInput.addEventListener('keydown', (e: KeyboardEvent) => {
    ...
    const items = autocompleteList.querySelectorAll<HTMLElement>('.autocomplete-list-item');
  ...
  document.addEventListener('click', (e) => {
    if (!autocompleteList.contains(e.target as Node) && e.target !== addressInput) {
      closeDropdown();
    }
  });
}
