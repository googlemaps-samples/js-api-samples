...
const App = () => (React.createElement(APIProvider, { apiKey: API_KEY, libraries: ['maps', 'places', 'marker', 'geometry', 'core'] },
    React.createElement(PlacesSearchLayout, null)));
const PlacesSearchLayout = () => {
    const [selectedType, setSelectedType] = useState('');
    const [places, setPlaces] = useState([]);
    const [selectedPlace, setSelectedPlace] = useState(null);
    const placeSearchRef = useRef(null);
    return (React.createElement("div", { className: "places-ui-kit" },
        React.createElement("div", { className: "place-list-wrapper", ref: placeSearchRef }),
        React.createElement("div", { className: "map-container" },
            React.createElement(Map, { defaultCenter: { lat: 48.8566, lng: 2.3522 }, defaultZoom: 16, mapId: "DEMO_MAP_ID", clickableIcons: false, onClick: () => {
                    setSelectedPlace(null);
                } },
                React.createElement(PlaceSearchController, { placeSearchRef: placeSearchRef, selectedType: selectedType, setPlaces: setPlaces, setSelectedPlace: setSelectedPlace, selectedPlace: selectedPlace }),
                places.map((place) => place.location && (React.createElement(AdvancedMarker, { key: place.id, position: place.location, onClick: () => {
                        console.log('Marker clicked:', place);
                        setSelectedPlace(place);
                    } }))))),
        React.createElement("div", { className: "controls" },
            React.createElement("select", { name: "types", className: "type-select", value: selectedType, onChange: (e) => {
                    setSelectedType(e.target.value);
                } },
                React.createElement("option", { value: "" }, "Select a place type"),
                React.createElement("option", { value: "cafe" }, "Cafe"),
                React.createElement("option", { value: "restaurant" }, "Restaurant"),
                React.createElement("option", { value: "electric_vehicle_charging_station" }, "EV charging station")))));
};
