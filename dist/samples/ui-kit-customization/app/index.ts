...
const contentElemContainer = document.querySelector(
  ".content-element-container") as any;
...
const styleText = document.querySelectorAll(".style-config input[type='text']") as any;
...
let placeElement, placeRequest, gmpPlaceAll, gmpPlaceStandard, gmpContentConfig,
  ...
  for (let property in settings.contents) {
    if (settings.contents[property]) {
      ...
      if (!gmpContentConfig.querySelector(`gmp-place-${property}`)) {
        gmpContentConfig.appendChild(document.createElement(
          `gmp-place-${property}`));
      }
    } else {
      //remove content
      if (gmpContentConfig.querySelector(`gmp-place-${property}`)) {
        gmpContentConfig.removeChild(gmpContentConfig.querySelector(
          `gmp-place-${property}`));
      }
    }
  }
  ...
  for (let property in settings.extraContents) {
    if (settings.extraContents[property]) {
      ...
      if (!gmpContentConfigFull.querySelector(`gmp-place-${property}`)) {
        gmpContentConfigFull.appendChild(document.createElement(
          `gmp-place-${property}`));
      }
    } else {
      //remove content
      if (gmpContentConfigFull.querySelector(`gmp-place-${property}`)) {
        gmpContentConfigFull.removeChild(gmpContentConfigFull.querySelector(
          `gmp-place-${property}`));
      }
    }
  }
